...
| Treehouse adapter | Isolated worker `HOME`, worker-owned repo, worker-owned pool root | User Treehouse config/hooks, GitHub tokens, provider credentials | Treehouse invokes git and manages worktree metadata in the worker-owned store |
